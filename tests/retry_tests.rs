...
use rota::retry::{
    ExponentialBackoff, FixedRetry, NoRetry, RetryOnError, RetryPolicy,
    is_transient_error,
};
...
fn no_retry_never_retries() {
    ...
    assert_eq!(policy.should_retry(100, &Error::Backend("any".into())), None);
}
...
fn fixed_retry_with_default_attempts() {
    ...
    assert_eq!(policy.should_retry(1, &Error::Backend("test".into())), Some(Duration::from_millis(100)));
    assert_eq!(policy.should_retry(2, &Error::Backend("test".into())), Some(Duration::from_millis(100)));
    assert_eq!(policy.should_retry(100, &Error::Backend("test".into())), Some(Duration::from_millis(100)));
}

#[tokio::test]
async fn fixed_retry_with_max_attempts() {
    let policy = FixedRetry::new(Duration::from_millis(50)).with_max_attempts(3);
    assert_eq!(policy.should_retry(1, &Error::Backend("test".into())), Some(Duration::from_millis(50)));
    assert_eq!(policy.should_retry(2, &Error::Backend("test".into())), Some(Duration::from_millis(50)));
...
fn fixed_retry_with_zero_delay() {
    ...
    assert_eq!(policy.should_retry(1, &Error::Backend("test".into())), Some(Duration::from_millis(0)));
}
...
fn exponential_backoff_grows() {
    ...
    let d1 = policy.should_retry(1, &Error::Backend("test".into())).unwrap();
    let d2 = policy.should_retry(2, &Error::Backend("test".into())).unwrap();
    let d3 = policy.should_retry(3, &Error::Backend("test".into())).unwrap();
...
fn exponential_backoff_respects_max_delay() {
    let policy = ExponentialBackoff::new(Duration::from_millis(100))
        ...
        .with_jitter(false);
    let d = policy.should_retry(10, &Error::Backend("test".into())).unwrap();
...
async fn exponential_backoff_with_max_attempts() {
    ...
    assert!(policy.should_retry(1, &Error::Backend("test".into())).is_some());
    assert!(policy.should_retry(2, &Error::Backend("test".into())).is_some());
    assert!(policy.should_retry(3, &Error::Backend("test".into())).is_none());
    assert!(policy.should_retry(10, &Error::Backend("test".into())).is_none());
}
...
fn exponential_backoff_with_jitter() {
    ...
    let policy = ExponentialBackoff::new(Duration::from_millis(10))
        .with_jitter(true);
    for _ in 0..10 {
        let d = policy.should_retry(1, &Error::Backend("test".into())).unwrap();
        // For attempt 1, base is 10ms * 2.0 = 20ms, with jitter 0.5x to 1.0x
        // So delay is between 10ms and 20ms
        assert!(d >= Duration::from_millis(5), "d ({:?}) should be >= 5ms", d);
        assert!(d <= Duration::from_millis(25), "d ({:?}) should be <= 25ms", d);
    }
}
...
fn exponential_backoff_without_jitter_is_deterministic() {
    let policy = ExponentialBackoff::new(Duration::from_millis(100))
        .with_jitter(false);
    let d1 = policy.should_retry(1, &Error::Backend("test".into())).unwrap();
    let d2 = policy.should_retry(1, &Error::Backend("test".into())).unwrap();
...
fn retry_on_error_predicate_false() {
    ...
    assert!(policy.should_retry(1, &Error::Backend("test".into())).is_none());
}
...
fn retry_on_error_delegates_to_inner() {
    let policy = RetryOnError::new(
        FixedRetry::new(Duration::from_millis(50)),
        |_| true,
    );
